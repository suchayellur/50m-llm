...
from data.datasets import choose_text_stream, token_batches
...
def main():
    ...
    text_iter = choose_text_stream(args.dataset, args.local_files)
    ...
    for step in trange(start_step, args.steps, initial=start_step, total=args.steps):
        ...
        if (step + 1) % args.save_every == 0 or (step + 1) == args.steps:
            ...
            save_checkpoint(output_dir / f"checkpoint-{step + 1}", model, optimizer, scheduler, step + 1, meta)
